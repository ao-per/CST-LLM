...
def process_folder(folder_path, output_file):
    ...
    # 表头只保留文件名和三阶总和（逗号分隔）
        f_out.write("文件名,三阶总和\n")
        
        total_all = 0.0  # 用于累加所有文件的三阶总和
        
        for csv_file in csv_files:
            file_path = os.path.join(folder_path, csv_file)
            
            try:
                sum_errors = 0  # 计算当前文件的6、8、10阶总误差之和
                for order in orders:
                    _, _, total_err = CST_fitting_error(file_path, order)
                    sum_errors += total_err
                
                # 写入当前文件的结果
                f_out.write(f"{csv_file},{sum_errors:.6f}\n")
                print(f"处理完成: {csv_file}")
                
                # 累加当前文件的三阶总和到总合计
                total_all += sum_errors
                
            except Exception as e:
                print(f"处理失败 {csv_file}: {str(e)}")
                f_out.write(f"{csv_file},ERROR: {str(e)[:50]}\n")
        
        # 所有文件处理完毕后，写入总和（用空文件名区分数据行）
        f_out.write(f",所有文件三阶总和总计:{total_all:.6f}\n")
