...
def CST_parameterization_with_TE_thickness(airfoil_file, N):
    ...
    # 对归一化后的x坐标进行特征转换
    x_features_upper = t_x(x_norm_upper)
    x_features_lower = t_x(x_norm_lower)
    ...
    # 构建类别函数 C(x) = t_x^N1 * (1-t_x)^N2
    C_upper = (x_features_upper)**N1 * (1 - x_features_upper)**N2
    C_lower = (x_features_lower)**N1 * (1 - x_features_lower)**N2

    # 构建伯恩斯坦多项式基函数（形状函数的基础）
    A_upper = np.zeros((len(x_features_upper), N+1))
    A_lower = np.zeros((len(x_features_lower), N+1))

    for i in range(N+1):
        # 伯恩斯坦多项式项：comb(N,i) * t_x^i * (1-t_x)^(N-i)
        A_upper[:, i] = comb(N, i) * (x_features_upper)**i * (1 - x_features_upper)**(N-i)
        A_lower[:, i] = comb(N, i) * (x_features_lower)**i * (1 - x_features_lower)**(N-i)
